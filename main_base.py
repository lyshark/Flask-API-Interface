...
def Send_Msg(mobile,msg):
    ...
    requests.post(url,timeout=5,data={"mobile": mobile,"message": msg})
                ...
                if (each[0] == key):
                    ...
                    for each in ref_data:
                        ...
                        if each[1] == "未授权用户" and each[0] == mobile:
                            msg_code = generate_random_str(4)
                            ...
                            return_dict = {'status': '0', 'message': '已授权用户无需继续授权'}
                            return json.dumps(return_dict, ensure_ascii=False)
                else:
                    return_dict = {'status': '0', 'message': '请先登录'}
                    return json.dumps(return_dict, ensure_ascii=False)
        else:
